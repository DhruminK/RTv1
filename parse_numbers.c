...
/*   Updated: 2019/05/21 16:04:28 by dkhatri          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
double		*ft_process_points(char **str, int num, int *res)
{
	...
	if (num != 1)
	{
		if (!(tmp = ft_strdup(s[0] + 1)) || !ft_strdel(&(s[0])))
			return (0);
		s[0] = tmp;
...
int			ft_parse_3points(char *s1, char *line, t_vec3 *vec)
{
	...
	if (!s1 || !line || !(s = ft_strsplit(line, ':')))
		return (-1);
	if ((ret = ft_process_strarr(s, s1, 3)) < 1 && ft_strarrdel(&s))
		return (ret);
	if ((!(sc = ft_strsplit(s[1], ',')) \
			|| !(d = ft_process_points(sc, 3, &ret)))
			&& ft_strarrdel(&s) && ft_strarrdel(&sc))
		return (ret < 1 ? ret : -1);
...
int			ft_parse_2points(char *s1, char *line, t_vec2 *vec)
{
	...
	if (!s1 || !line || !(s = ft_strsplit(line, ':')))
		return (-1);
	if ((ret = ft_process_strarr(s, s1, 2)) < 1 && ft_strarrdel(&s))
		return (ret);
	if ((!(sc = ft_strsplit(s[1], ',')) \
...
int			ft_parse_1point(char *s1, char *line, int hex, void *val)
{
	...
	if (!s1 || !line || !(s = ft_strsplit(line, ':')))
		return (-1);
	if ((ret = ft_process_strarr(s, s1, 1)) < 1 && ft_strarrdel(&s))
		return (ret);
